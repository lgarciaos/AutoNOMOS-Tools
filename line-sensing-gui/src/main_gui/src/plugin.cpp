...
void MyPlugin::onTopicChanged(int index)
{
  ...
  if (!topic.isEmpty())
  {
    ...
    try {
      subscriber_ = it.subscribe(topic.toStdString(), 1, &MyPlugin::callbackImage, this, hints);
...
void MyPlugin::onZoom1(bool checked)
{
  if (checked)
  {
...
void MyPlugin::onDynamicRange(bool checked)
{
  ui_.max_range_double_spin_box->setEnabled(!checked);
}

void MyPlugin::updateNumGridlines()
{
...
void MyPlugin::onMousePublish(bool checked)
{
...
void MyPlugin::onMouseLeft(int x, int y)
{
...
void MyPlugin::onPubTopicChanged()
{
  pub_topic_custom_ = !(ui_.publish_click_location_topic_line_edit->text().isEmpty());
  onMousePublish(ui_.publish_click_location_check_box->isChecked());
}

void MyPlugin::onHideToolbarChanged(bool hide)
{
  ui_.toolbar_widget->setVisible(!hide);
}

void MyPlugin::invertPixels(int x, int y)
{
...
QList<int> MyPlugin::getGridIndices(int size) const
{
  QList<int> indices;
  ...
  catch (cv_bridge::Exception& e)
  {
    try
    {
      ...
      if (msg->encoding == "CV_8UC3")
      {
        ...
        qWarning("MyPlugin.callback_image() could not convert image from '%s' to 'rgb8' (%s)", msg->encoding.c_str(), e.what());
        ui_.image_frame->setImage(QImage());
        return;
      }
    }
    ...
    {
      qWarning("MyPlugin.callback_image() while trying to convert image from '%s' to 'rgb8' an exception was thrown (%s)", msg->encoding.c_str(), e.what());
      ui_.image_frame->setImage(QImage());
      return;
    }
  }
...
void MyPlugin::set_sliders_min_val(int min_val)
{
...
void MyPlugin::set_sliders_max_val(int max_val)
{
...
void MyPlugin::right_scroll_changed()
{
  ui_.label_right->setText(QString("<html><head/><body><p align=\"center\">Right - %1</p></body></html>").arg(ui_.horizontalScrollBar_right->value()));
}

void MyPlugin::center_scroll_changed()
{
...
void MyPlugin::left_scroll_changed()
{
...
PLUGINLIB_EXPORT_CLASS(main_gui::MyPlugin, main_gui::MyPlugin)
PLUGINLIB_DECLARE_CLASS(main_gui, MyPlugin, main_gui::MyPlugin, rqt_gui_cpp::Plugin)